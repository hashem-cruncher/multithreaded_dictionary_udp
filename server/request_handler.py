...
class RequestHandlerPool:
    ...
    def start(self):
        ...
        for i in range(self.num_handlers):
            handler = threading.Thread(
                target=self._handler_thread, args=(i,), name=f"Handler-{i}"
            )
            handler.daemon = True
            handler.start()
    ...
    def _handler_thread(self, handler_id):
        ...
        while self.running:
            try:
                ...
                try:
                    data, client_address, timestamp = self.request_queue.get(
                        timeout=0.5
                    )
                except:
                    continue
                ...
                if queue_time > 1.0:
                    logger.warning(
                        f"Request from {client_address} spent {queue_time:.2f}s in queue"
                    )

                # Process the request
                logger.debug(
                    f"Handler {handler_id} processing request from {client_address}"
                )
        ...
        try:
            ...
            if action == "lookup":
                ...
                if definition:
                    self._send_response(
                        client_address,
                        {"status": "found", "word": word, "definition": definition},
                    )
                else:
                    self._send_response(
                        client_address, {"status": "not_found", "word": word}
                    )
            elif action == "ping":
    ...
    def _send_error_response(self, client_address, error_message):
        ...
        """
        self._send_response(
            client_address, {"status": "error", "message": error_message}
        )
